...
def computePrior(labels, W=None):
    ...
    for idx, clazz in enumerate(classes):
        clazz_indices = np.where(labels == clazz)[0]
        Wk = W[clazz_indices]
        prior[idx] = np.sum(Wk) / w_sum  # false --> Wk.shape[0]
    # ==========================
...
def mlParams(X, labels, W=None):
    ...
    # ==========================
    for idx, clazz in enumerate(classes):  # TODO can this be written in one line?
        clazz_indices = np.where(labels == clazz)[0]
        Dk = X[clazz_indices]
        Wk = W[clazz_indices]
        mu[idx] = np.sum(Wk * Dk, axis=0) / np.sum(Wk)
        sigma[idx] = np.diag(np.sum(Wk * (Dk - mu[idx]) ** 2, axis=0) / np.sum(Wk))
    # ==========================
...
def classifyBayes(X, prior, mu, sigma):
    ...
    # ==========================
    for idx, (clazz_mu, clazz_sigma) in enumerate(zip(mu, sigma)):
        x_minus_mu = X - clazz_mu
        logProb[idx] = (
                -np.sum((x_minus_mu @ np.linalg.inv(clazz_sigma)) * x_minus_mu, axis=-1) / 2
                - np.log(np.linalg.det(clazz_sigma)) / 2
                + np.log(prior[idx])
        )
    # x_minus_mu = X[:, np.newaxis, :] - mu
    # logProb = (
    #         -(np.sum(np.sum(x_minus_mu[:, :, :, np.newaxis] * np.linalg.inv(sigma), axis=-1) * x_minus_mu, axis=-1)) / 2
    #         - np.log(np.linalg.det(sigma)) / 2
    #         + np.log(prior).reshape(-1)
    # ).T
    # ==========================
...
# datasets = "iris", "vowel", "wine", "olivetti"
# for dataset in datasets:
#     print(f"***Dataset {dataset}***")
#     testClassifier(BayesClassifier(), dataset=dataset, split=0.7)


# plotBoundary(BayesClassifier(), dataset='vowel', split=0.7)  # Does not make much sense to plot vowel, its not 2d..
...
def trainBoost(base_classifier, X, labels, T=10):
    ...
    for i_iter in range(0, T):
        ...
        wCur = wCur * np.exp(np.where(vote == labels, -1, 1) * alpha).reshape(-1, 1)
        if np.sum(wCur) == 0:
            wCur = np.ones((Npts, 1)) / float(Npts)
...
def classifyBoost(X, classifiers, alphas, Nclasses):
    ...
    if Ncomps == 1:
        ...
        for alpha, classifier in zip(alphas, classifiers):
            for i, c in enumerate(classifier.classify(X)):
                votes[i][c] += alpha
        # ==========================
...
# for dataset in datasets:
#     print(f"***Dataset {dataset}***")
#     testClassifier(BoostClassifier(BayesClassifier(), T=10), dataset=dataset, split=0.7)

# plotBoundary(BoostClassifier(BayesClassifier()), dataset='vowel', split=0.7)  # Does not make much sense to plot vowel, its not 2d..
...
#     testClassifier(DecisionTreeClassifier(), dataset=dataset, split=0.7)
#     print("BOOSTED Decision tree classifier")
#     testClassifier(BoostClassifier(DecisionTreeClassifier(), T=10), dataset=dataset,split=0.7)
#     print("\n\t\n.\n\n\n")
